...
class QandATableViewController: TextTableViewController {
    ...
    var qanda: QandA {
        get {
            ...
            return QandA(question: data?.first?.first ?? "",
                         answers: answers)
        }
        set {
        ...
        didSet {
            ...
            while answerIndex < qanda.answers.count {
                if qanda.answers[answerIndex] == answer {
                    ...
                    _ = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(chooseAnswer(_:)), userInfo: indexPath , repeats: false)
                    break
                }
                answerIndex += 1
            }
    ...
    override func createTextViewForIndexPath(indexPath: IndexPath?) -> UITextView {
        let textView = super.createTextViewForIndexPath(indexPath: indexPath)
...
/* zap
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) -> UITableViewCellEditingStyle  {
